...
app.layout = html.Div([ #TASK 2.1 Add title
    to the dashboardhtml.H1('Automobiles Sales Statistics Dashboard', style={'textAlign': 'center','color':'#50D36','fontsize':24}),
...
def update_output(selected_year, selected_stat):
    if selected_year and selected_stat:
        return f"You selected {selected_year} with {selected_stat}"
    elif selected_year:
        return f"You selected {selected_year}"
    elif selected_stat:
        return f"You selected {selected_stat}"
    else:
...
def update_output_container(selected_year, selected_stat):
    if selected_stat == 'Recession Period Statistics':
        # Filter the data for recession periods
        recession_data = data[data['Recession'] == 1]

        # If you want to filter further by year:
        if selected_year:
            recession_data = recession_data[recession_data['Year'] == selected_year]

        # Example: return a Plotly figure
        fig = px.bar(
            recession_data,
            x='Category',
            y='Value',
            title=f'Recession Statistics for {selected_year}' if selected_year else 'Recession Statistics'
        )
        return dcc.Graph(figure=fig)

    else:
        return "Please select 'Recession Period Statistics' to view data."
...
#Plot 2 Calculate the average number of vehicles sold by vehicle type       
        ...
        average_sales = recession_data.groupby('Vechicle_Type')['Automobiles_sales'].mean().reset_inde()               
        R_chart2  = dcc.Graph(
            figure=px.bar(average_sales
            x='Vehicle_type',
            y='Autombiles_Sales',
            title="Average Automobiles Sales by Vehicle Type over Recession Period "))
